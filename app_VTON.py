...
with image_blocks as demo:
    gr.Markdown("## AlanaIa Virtual Try-on")
    gr.Markdown("Esta es una demostración del modelo de prueba virtual AlanaIa. Sube una imagen de una persona y una prenda para ver a la persona vistiendo la prenda.")
    with gr.Row():
        with gr.Column():
            imgs = gr.ImageEditor(sources='upload', type="pil", label='Humano. Mascara con lápiz o usa auto-mascarado', interactive=True , height=550)
            with gr.Row():
                category = gr.Radio(choices=["upper_body", "lower_body", "dresses"], label="Selecciona la categoría de la prenda", value="upper_body")
                is_checked = gr.Checkbox(label="Yes", info="Use auto-generated mask (Takes 5 seconds)",value=True)
            with gr.Row():
                is_checked_crop = gr.Checkbox(label="Yes", info="Usar recorte automático y redimensionado",value=True)
        ...
        with gr.Column():
            garm_img = gr.Image(label="Prenda", sources='upload', type="pil")
            with gr.Row(elem_id="prompt-container"):
                with gr.Row():
                    prompt = gr.Textbox(placeholder="Descripción de la prenda (ej. Camisetas de manga corta y cuello redondo)", show_label=False, elem_id="prompt")
            example = gr.Examples(
                inputs=garm_img,
                examples_per_page=8,
        ...
        with gr.Column():
            ...
            # image_out = gr.Image(label="Output", elem_id="output-img", height=400)
                masked_img = gr.Image(label="Salida de imagen enmascarada", elem_id="masked-img",show_share_button=False)
            with gr.Row():
                btn_open_outputs = gr.Button("Abrir carpeta de salidas")
                btn_open_outputs.click(fn=open_folder)
        with gr.Column():
            with gr.Row():
            # image_out = gr.Image(label="Output", elem_id="output-img", height=400)
                image_gallery = gr.Gallery(label="Imágenes Generadas", show_label=True)
            with gr.Row():
                try_button = gr.Button(value="Probar")
                denoise_steps = gr.Number(label="Pasos de eliminación de ruido", minimum=20, maximum=120, value=30, step=1)
                seed = gr.Number(label="Seed", minimum=-1, maximum=2147483647, step=1, value=1)
                is_randomize_seed = gr.Checkbox(label="Aleatorizar semilla para cada imagen generada", value=True)  
                number_of_images = gr.Number(label="Número de imágenes a generar (comenzará desde tu semilla de entrada y se incrementará en 1)", minimum=1, maximum=9999, value=1, step=1)
